...
import unittest
import redis
from contest.packages.message_parsers.__default_parser import default_parser
import json
...
class default_parser_test(unittest.TestCase):
    ...
    def test_parse_Normal_Multi_Request_Message(self):
        ...
        self.assertFalse(self.list_has_duplicate_items(result_message[u'items']),
            'the list has duplicate entries for recommendation items' + str(result_message[u'items']))
        print json.dumps(result_message)
